...
test('MV3 popup requests optional access on clicks, never renders URLs, explicit candidate and receiver',async()=>{
 const manifest=JSON.parse(await read('manifest.json'));assert.equal(manifest.manifest_version,3);assert.equal(manifest.background.service_worker,'worker.mjs');assert.equal(manifest.background.type,'module');assert.deepEqual(manifest.optional_host_permissions,['http://*/*','https://*/*']);assert.ok(!manifest.host_permissions);for(const p of ['activeTab','scripting','webNavigation','webRequest','nativeMessaging','alarms'])assert.ok(manifest.permissions.includes(p));
 const html=await read(manifest.action.default_popup);assert.match(html,/popup.js/);assert.match(html,/reload/i);assert.match(html,/TV video and audio/i);
 const elements=new Map([...html.matchAll(/id="([^"]+)"/g)].map(m=>[m[1],{value:'',textContent:'',disabled:false,replaceChildren(){},append(){}}]));const actions=[];let requests=0;
 ...
 assert.equal(elements.get('candidate').value,'');assert.equal(elements.get('receiver').value,'');
